...
export class DataSyncService {
  ...
  private async syncEntity<T extends object>(
    ...
    try {
      ...
      // Fallback si la propiedad no existe
      const result =
        resp[resultKey] ??
        resp['createActividad'] ??
        resp['CreateActividad'] ??
        undefined;
  ...
  private buildSesionInput(s: SesionesDB): Record<string, unknown> {
    return {
      ...
      fecha_modificacion: this.toDateOnly(s.fecha_modificacion),
      fecha_creacion: this.toDateOnly(s.fecha_creacion),
      fecha_actividad: this.toDateOnly(s.fecha_actividad),
  ...
  async crearActividades(id_actividad: string): Promise<void> {
    await this.syncEntity<ActividadesDB>(
      ...
      (a) => ({
        ...
        fecha_modificacion: this.toDateOnly(a.fecha_modificacion),
        fecha_creacion: this.toDateOnly(a.fecha_creacion),
        fecha_actividad: this.toDateOnly(a.fecha_actividad),
  ...
  async crearSesiones(id_sesion: string): Promise<void> {
    await this.syncEntity<SesionesDB>(
      ...
      this.buildSesionInput.bind(this),
      this.CREATE_SESION,
      'createSesion',
