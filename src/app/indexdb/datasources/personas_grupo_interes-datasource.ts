...
export class Personas_grupo_interesDataSource {
  async getAll(): Promise<Personas_grupo_interesDB[]> {
    return await indexDB.personas_grupo_interes.toArray();
  }

  async getById(id: string): Promise<Personas_grupo_interesDB | undefined> {
    return await indexDB.personas_grupo_interes.get(id);
  }

  async create(data: Personas_grupo_interesDB): Promise<string> {
    return await indexDB.personas_grupo_interes.add(data);
  }

  async update(
    id: string,
    changes: Partial<Personas_grupo_interesDB>,
  ): Promise<number> {
  ...
  async bulkAdd(data: Personas_grupo_interesDB[]): Promise<void> {
