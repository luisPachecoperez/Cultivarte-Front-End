...
export class Personas_sedesDataSource {
  async getAll(): Promise<Personas_sedesDB[]> {
    return await indexDB.personas_sedes.toArray();
  }

  async getById(id: string): Promise<Personas_sedesDB | undefined> {
    return await indexDB.personas_sedes.get(id);
  }

  async create(data: Personas_sedesDB): Promise<string> {
    return await indexDB.personas_sedes.add(data);
  }

  async update(
    id: string,
    changes: Partial<Personas_sedesDB>,
  ): Promise<number> {
  ...
  async bulkAdd(data: Personas_sedesDB[]): Promise<void> {
