...
describe('✅ CalendarComponent (Jest 90%)', () => {
  ...
  it('🛠 eliminarSesionDelCalendario() elimina por id cuando es UUID válido', () => {
    const uuid = '123e4567-e89b-12d3-a456-426614174000';
    component.eventosCalendario = [
      { id_sesion: uuid, title: 'EventoUUID' } as any,
    ];

    component.eliminarSesionDelCalendario(uuid);
  ...
  it('🧩 onAccionSeleccionada() debe manejar correctamente el operador ??', () => {
    // Caso 1: respuesta.id_sesion está definido
    const respuesta = { id_sesion: 'S123' };
    const resultado = respuesta.id_sesion ?? 'valor_por_defecto';
    expect(resultado).toBe('S123');

    // Caso 2: respuesta.id_sesion es null
    const respuestaNull = { id_sesion: null };
    const resultadoNull = respuestaNull.id_sesion ?? 'valor_por_defecto';
    expect(resultadoNull).toBe('valor_por_defecto');

    // Caso 3: respuesta.id_sesion es undefined
    const respuestaUndefined = { id_sesion: undefined };
    const resultadoUndefined = respuestaUndefined.id_sesion ?? 'valor_por_defecto';
    expect(resultadoUndefined).toBe('valor_por_defecto');
  });
  ...
  it('🧩 agregarOActualizarEvento() debe aceptar múltiples sesiones', () => {
    ...
    if (Array.isArray(component.calendarOptions.events)) {
      expect(component.calendarOptions.events.length).toBe(2);
    } else {
      fail('calendarOptions.events no es un arreglo');
    }
  });
  ...
  it('🧩 handleEventClick() cuando no hay startStr ni extendedProps.desde deja fecha_actividad vacía', () => {
    ...
    expect(sesion.fecha_actividad).toBe('2025-08-11');
    expect(sesion.hora_inicio).toBe('14:00');
    expect(sesion.hora_fin).toBe('15:00');
  });
  ...
  it('usa "" como valor por defecto si e.start es undefined', () => {
    component.calendarOptions.events = [{
      id: 'E1',
      title: 'Actividad 1',
      start: undefined,
    ...
    const sesiones = component.mapEventosRelacionados('Actividad 1');

    expect(sesiones[0].hora_inicio).toBe('');
  });

  it('🧪 mapEventosRelacionados usa "" para fecha_actividad cuando start es undefined', () => {
    component.calendarOptions.events = [
      {
    ...
    const sesiones = component.mapEventosRelacionados('Actividad sin fecha');

    expect(sesiones[0].fecha_actividad).toBe('');
  });

  it('🧪 mapEventosRelacionados aplica el fallback "" cuando start no provee fecha', () => {
    component.calendarOptions.events = [
      {
        title: 'Actividad fallback',
        start: { split: () => [] } as any,
        end: undefined,
        id_actividad: 'A3',
        id_sesion: 'S3',
    ...
    const sesiones = component.mapEventosRelacionados('Actividad fallback');

    expect(sesiones[0].fecha_actividad).toBe('');
  });

  it('🧪 cerrarRegistroBeneficiario() oculta el registro', () => {
    component.mostrarRegistroBeneficiario = true;
    component.cerrarRegistroBeneficiario();
    expect(component.mostrarRegistroBeneficiario).toBe(false);
  });
  it('🧪 abrirEdicion() asigna "" cuando id_actividad es undefined', () => {
    component.calendarOptions.events = [
      {
        title: 'EventoSinActividad',
        start: '2025-09-01T08:00',
        end: '2025-09-01T09:00',
        id_sesion: 'S20',
    ...
    expect(component.eventoSeleccionado?.id_actividad).toBe('');
  });

  it('🧪 abrirEdicion() asigna "" cuando id_sesion es undefined', () => {
    component.calendarOptions.events = [
      {
        id_actividad: 'A1',
        id_sesion: 'S1',
        title: 'EventoSinSesion',
        start: '2025-09-01T08:00',
        end: '2025-09-01T09:00',
      } as any,
    ];
    ...
    expect(component.eventoSeleccionado?.id_sesion).toBe('');
  });

  it('🧪 handleEventClick() asigna "" cuando no hay id_sesion ni event.id', () => {
    const eventArg = {
      event: {
        id: undefined,
        title: 'EventoSinIdSesion',
        startStr: '2025-04-01T08:00',
        endStr: '2025-04-01T09:00',
        extendedProps: {
          id_actividad: 'A4',
          asistentes_evento: 3,
        },
  ...
  it('🧪 handleEventClick() asigna "" a nombre_actividad cuando title es undefined', () => {
    const eventArg = {
      event: {
        id: 'S100',
        title: undefined,
        startStr: '2025-04-02T08:00',
        endStr: '2025-04-02T09:00',
        extendedProps: {
          id_actividad: 'A100',
          id_sesion: 'S100',
        },
  ...
  it('🧪 handleEventClick() mantiene "" cuando title es cadena vacía', () => {
    const eventArg = {
      event: {
        id: 'S200',
        title: '',
        startStr: '2025-04-03T08:00',
        endStr: '2025-04-03T09:00',
        extendedProps: {
          id_actividad: 'A200',
          id_sesion: 'S200',
        },
  ...
  it('🧪 handleEventClick() asigna "" a fecha_actividad cuando no hay fecha disponible', () => {
    const eventArg = {
      event: {
        id: 'S300',
        title: 'EventoSinFecha',
        startStr: '',
        endStr: '',
        extendedProps: {
          id_actividad: 'A300',
  ...
  it('🧪 abrirRegistroBeneficiario() muestra el registro', () => {
    component.mostrarRegistroBeneficiario = false;

    component.abrirRegistroBeneficiario();

    expect(component.mostrarRegistroBeneficiario).toBe(true);
  });
});