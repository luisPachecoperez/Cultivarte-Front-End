...
describe('✅ AsistenciaFotograficaComponent (Jest)', () => {
  it('no debe continuar si evento es undefined', async () => {
    fixture.componentRef.setInput('evento', undefined);
    await component.guardar();
    expect(asistenciaServiceMock.guardarAsistenciaFotografica).not.toHaveBeenCalled();
  });

  it('debe obtener valores del formulario si evento existe', async () => {
    ...
    asistenciaServiceMock.guardarAsistenciaFotografica.mockResolvedValueOnce({ exitoso: 'S' });

    await component.guardar();

    const callArg = asistenciaServiceMock.guardarAsistenciaFotografica.mock.calls[0][0];
    expect(callArg.nro_asistentes).toBe(5);
    expect(callArg.descripcion).toBe('Test');
  });
  it('debe asignar 0 a nro_asistentes si numeroAsistentes es null', async () => {
    const mockSesion: Sesiones = { id_sesion: '1', id_actividad: '2' } as any;
    fixture.componentRef.setInput('evento', mockSesion);

    // Deshabilitamos el validador required para este test
    component.asistenciaForm.get('numeroAsistentes')?.clearValidators();
    component.asistenciaForm.get('numeroAsistentes')?.updateValueAndValidity();

    component.asistenciaForm.patchValue({
      numeroAsistentes: null,
      descripcion: 'Test null',
      foto: null,
    });
    ...
    const callArg = asistenciaServiceMock.guardarAsistenciaFotografica.mock.calls[0][0];
    expect(callArg.nro_asistentes).toBe(0);
  });

  it('debe asignar array vacío si data.sedes es undefined', async () => {
    const mockSesion: Sesiones = { id_sesion: '1', id_actividad: '2' } as any;
    fixture.componentRef.setInput('evento', mockSesion);
  ...
  it('💾 guardar() debe guardar correctamente y emitir eventos', async () => {
    ...
    it('debe pasar "" si evento no tiene id_sesion', async () => {
      fixture.componentRef.setInput('evento', { id_actividad: 'X' } as any); // sin id_sesion
      asistenciaServiceMock.obtenerDetalleAsistencia.mockResolvedValueOnce({ sedes: [] });
    ...
    it('no debe llamar al servicio si evento es undefined', async () => {
      fixture.componentRef.setInput('evento', undefined);
      asistenciaServiceMock.obtenerDetalleAsistencia.mockReset();
