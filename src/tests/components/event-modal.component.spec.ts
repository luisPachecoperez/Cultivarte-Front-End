...
describe('EventModalComponent', () => {
  ...
  describe('eliminarEvento', () => {
    ...
    it('debe usar "sin nombre" como valor por defecto si nombre_actividad es null o undefined', async () => {
      setInput(component, 'evento', { id_actividad: '123', nombre_actividad: undefined } as Actividades);
      snackbarService.confirm.mockReturnValueOnce(of(true));

      const promise = component.eliminarEvento();
      snackbarService.resolveConfirm(true);
      await promise;

      expect(snackbarService.confirm).toHaveBeenCalledWith(
        '¿Deseas eliminar el Evento "sin nombre"?'
      );
    });

    it('debe mostrar "Eliminado correctamente" si el backend no envía un mensaje', async () => {
      setInput(component, 'evento', mockActividad);
      snackbarService.confirm.mockReturnValueOnce(of(true));
      eventModalService.eliminarEvento.mockResolvedValueOnce({ exitoso: 'S', mensaje: '' });

      const promise = component.eliminarEvento();
      snackbarService.resolveConfirm(true);
      await promise;

      expect(snackbarService.success).toHaveBeenCalledWith('Eliminado correctamente');
    });

    it('debe mostrar "No se pudo eliminar" si el backend devuelve exito = "N" y no envía un mensaje', async () => {
      setInput(component, 'evento', mockActividad);
      snackbarService.confirm.mockReturnValueOnce(of(true));
      eventModalService.eliminarEvento.mockResolvedValueOnce({ exitoso: 'N', mensaje: null });

      const promise = component.eliminarEvento();
      snackbarService.resolveConfirm(true);
      await promise;

      expect(snackbarService.error).toHaveBeenCalledWith('No se pudo eliminar');
    });

    it('debe mostrar el mensaje del error si ocurre un error inesperado', async () => {
      setInput(component, 'evento', mockActividad);
      snackbarService.confirm.mockReturnValueOnce(of(true));
      eventModalService.eliminarEvento.mockRejectedValueOnce(new Error('Error inesperado'));

      const promise = component.eliminarEvento();
      snackbarService.resolveConfirm(true);
      await promise;

      expect(snackbarService.error).toHaveBeenCalledWith('Error inesperado');
    });

    it('debe mostrar "Error eliminando el evento" si el error no es una instancia de Error', async () => {
      setInput(component, 'evento', mockActividad);
      snackbarService.confirm.mockReturnValueOnce(of(true));
      eventModalService.eliminarEvento.mockRejectedValueOnce('Error desconocido');

      const promise = component.eliminarEvento();
      snackbarService.resolveConfirm(true);
      await promise;

      expect(snackbarService.error).toHaveBeenCalledWith('Error eliminando el evento');
    });
  });
