...
describe('🧠 AsistenciaService (Refactor Completo)', () => {
  it('usa "" como valor por defecto si imagen es undefined en guardarAsistenciaFotograficaOffline', (done) => {
    const input: Sesiones = {
      id_sesion: 'S1',
      id_actividad: 'A1',
      // imagen está undefined
      descripcion: 'desc',
      nro_asistentes: 5,
    } as any;
    ...
    (service as any)
      .guardarAsistenciaFotograficaOffline(input)
      .subscribe(() => {
        const updateArgs = sesionesDS.update.mock.calls[0][1];
        expect(updateArgs.imagen).toBe('');
        done();
      });
  });
  it('usa "" como valor por defecto si id_sesion es undefined en guardarAsistenciaFotograficaOffline', (done) => {
    const input: Sesiones = {
      // id_sesion está undefined
      id_actividad: 'A1',
      imagen: 'img.png',
      descripcion: 'desc',
      nro_asistentes: 5,
    ...
    sesionesDS.getById.mockResolvedValue({ id_sesion: '' });
    sesionesDS.update.mockResolvedValue({ ok: true });

    (service as any)
      .guardarAsistenciaFotograficaOffline(input)
      .subscribe(() => {
        expect(sesionesDS.getById).toHaveBeenCalledWith('');
        done();
      });
  });
  it('usa "" como valor por defecto si imagen es undefined', (done) => {
    const input: Sesiones = {
      id_sesion: 'S1',
      id_actividad: 'A1',
      // imagen está undefined
    ...
    (service as any)
      ...
      .subscribe(() => {
        const updateArgs = sesionesDS.update.mock.calls[0][1];
        // Verifica que imagen se asigna como ''
        expect(updateArgs.imagen).toBe('');
        done();
      });
  });
  it('usa 0 como valor por defecto si nro_asistentes es undefined', (done) => {
    const input: Sesiones = {
      id_sesion: 'S1',
      id_actividad: 'A1',
      imagen: 'img.png',
      descripcion: 'desc',
      // nro_asistentes está undefined
    } as any;
    ...
    (service as any)
      ...
      .subscribe(() => {
        const updateArgs = sesionesDS.update.mock.calls[0][1];
        // Verifica que nro_asistentes se asigna como 0
        expect(updateArgs.nro_asistentes).toBe(0);
        done();
      });
  });
  ...
  it('⚠️ lanza error si update() falla offline', async () => {
    ...
    try {
      ...
      if (error instanceof Error) {
        expect(error.message).toContain('falló update');
      } else {
