...
describe('🧩 LoadIndexDBService (Jest)', () => {
  it('⚪ loadAsistenciasSede usa [] cuando response.getAsistenciasSede es undefined', async () => {
    const gql = TestBed.inject(GraphQLService) as any;
    const ds = TestBed.inject(AsistenciasDataSource) as any;
  ...
  it('⚪ loadPersonaProgramas usa [] cuando response.getPersonaProgramas es undefined', async () => {
    const gql = TestBed.inject(GraphQLService) as any;
    const personasProgramasDS = TestBed.inject(PersonasProgramasDataSource) as any;
    const bulkSpy = jest.spyOn(personasProgramasDS, 'bulkAdd');
    jest.spyOn(gql, 'query').mockReturnValueOnce(of({} as any));
    await TestBed.inject(LoadIndexDBService).loadPersonaProgramas();
    expect(bulkSpy).toHaveBeenCalledTimes(1);
    expect(bulkSpy.mock.calls[0][0]).toEqual([]); // rama nullish
  });
  it('⚪ loadPersonasSedes usa [] cuando response.getPersonasSedes es undefined', async () => {
    const gql = TestBed.inject(GraphQLService) as any;
    const personasSedesDS = TestBed.inject(PersonasSedesDataSource) as any;
    const bulkSpy = jest.spyOn(personasSedesDS, 'bulkAdd');
    jest.spyOn(gql, 'query').mockReturnValueOnce(of({} as any));
    await TestBed.inject(LoadIndexDBService).loadPersonasSedes();
    expect(bulkSpy).toHaveBeenCalledTimes(1);
    expect(bulkSpy.mock.calls[0][0]).toEqual([]); // rama nullish
  });
  it('⚪ loadPersonas usa [] cuando response.getPersonas es undefined', async () => {
    const gql = TestBed.inject(GraphQLService) as any;
    const personasDS = TestBed.inject(PersonasDataSource) as any;
    const bulkSpy = jest.spyOn(personasDS, 'bulkAdd');
    jest.spyOn(gql, 'query').mockReturnValueOnce(of({} as any));
    await TestBed.inject(LoadIndexDBService).loadPersonas();
    expect(bulkSpy).toHaveBeenCalledTimes(1);
    expect(bulkSpy.mock.calls[0][0]).toEqual([]); // rama nullish
  });
