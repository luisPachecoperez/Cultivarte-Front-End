...
describe('🧩 SnackbarYesNoService (Jest)', () => {
  ...
  describe('✅ success()', () => {
  it('🧪 usa duration=3000 por defecto en success()', (done) => {
    const message = 'Mensaje por defecto';
    // Llama success solo con el mensaje, sin duration
    const obs$ = service.success(message);

    setTimeout(() => {
      (service as any).confirmResult$.next(true);
    }, 0);
  ...
  describe('⚠️ warning()', () => {
  it('🧪 usa duration=3000 por defecto en warning()', (done) => {
    const message = 'Mensaje warning por defecto';
    // Llama warning solo con el mensaje, sin duration
    const obs$ = service.warning(message);
  ...
  describe('❌ error()', () => {
  it('🧪 usa duration=3000 por defecto en error()', (done) => {
    const message = 'Mensaje error por defecto';
    // Llama error solo con el mensaje, sin duration
    const obs$ = service.error(message);
