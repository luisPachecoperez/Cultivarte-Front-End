...
describe('🧩 GridSesionesService (Jest, Cobertura 95%)', () => {
  ...
  it('🆕 crea sesión con id_sesion vacío si no viene definido', async () => {
    loadIndexDB.ping.mockReturnValue(of('pong'));
    const mockResponse: GraphQLResponse = { exitoso: 'S', mensaje: 'OK' };
    graphQL.mutation.mockReturnValue(of({ updateSesiones: mockResponse }));
    ...
    const payloadSinId = {
      sesiones: {
        nuevos: [{ id_creado_por: 'U1' } as any],
        modificados: [],
        eliminados: [],
      },
    };

    await service.guardarCambiosSesiones(payloadSinId);

    expect(sesionesDS.create).toHaveBeenCalledWith(
      expect.objectContaining({ id_sesion: '' })
    );
  });

  it('🆙 actualiza sesión con id_sesion vacío si no viene definido en modificados', async () => {
    loadIndexDB.ping.mockReturnValue(of('pong'));
    const mockResponse: GraphQLResponse = { exitoso: 'S', mensaje: 'OK' };
    graphQL.mutation.mockReturnValue(of({ updateSesiones: mockResponse }));

    // modificado sin id_sesion
    const payloadSinId = {
      sesiones: {
        nuevos: [],
        modificados: [{ id_creado_por: 'U2' } as any],
        eliminados: [],
      },
    };
  ...
  it('🆕 (offline) crea sesión con id_sesion vacío si no viene definido', async () => {
    loadIndexDB.ping.mockReturnValue(of('offline'));
  ...
  it('🆙 (offline) actualiza sesión con id_sesion vacío si no viene definido en modificados', async () => {
    loadIndexDB.ping.mockReturnValue(of('offline'));
    ...
    expect(sesionesDS.update).toHaveBeenCalledWith(
      '',
      expect.objectContaining({ id_sesion: '', syncStatus: 'pending-update' })
    );
  });
