...
describe('SesionesDataSource (Jest)', () => {
  ...
  describe('🟢 update', () => {
    ...
    it('debe actualizar con changes.syncStatus = "synced"', async () => {
      (indexDB.sesiones.get as jest.Mock).mockReturnValue(
        dexiePromise({ ...mockSesion, syncStatus: 'otro' }),
      );
      const changes = { nombre_actividad: 'Modificada', syncStatus: 'synced' };
      await service.update('S1', changes);
      expect(indexDB.sesiones.update).toHaveBeenCalledWith('S1', changes);
    });
  });
